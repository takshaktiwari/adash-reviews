<div class="card border-0 shadow-sm rounded-4 h-100">
    <div class="card-body p-4 d-flex flex-column">
        ...
        <div class="d-flex flex-wrap align-items-center gap-3 mb-3">
            <img src="{{ $review->avatarUrl() }}" alt="{{ $review->name }}" class="rounded-circle flex-shrink-0"
                width="46" height="46" style="object-fit:cover;">
            <div class="flex-fill overflow-hidden">
                <div class="fw-bold text-truncate lh-sm">{{ $review->name }}</div>
                <div class="text-muted" style="font-size:0.7rem;">{{ $review->created_at->diffForHumans() }}</div>
            </div>
            <div class="d-flex gap-1">
                @for ($i = 1; $i <= 5; $i++)
                    <i class="bi my-auto {{ $i <= $review->rating ? 'bi-star-fill text-warning' : 'bi-star text-muted' }}"
